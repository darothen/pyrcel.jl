...
aer = AerosolSpecies("test", dist, n_bins, κₐ)
...
for r_dry in reverse(r_drys(aer))
...
## Total water volume
𝑉(r, r_dry, Ni) =  (4*π/3.0) * c.rho_w * Ni * (r^3 - r_dry^3)
wc₀ = sum(𝑉.(r0s, r_drys(aer), Ns(aer))) / ρ_air(T₀, P₀, 0.0)
wi₀ = 0.0

@printf "AEROSOL DISTRIBUTION\n"
for i = 1:length(aer.bins)
    @printf "%3.2e %8.1f\n" r_drys(aer)[i] Ns(aer)[i]
end
...
append!(y₀, r0s)
accom = 1.0
params = [r_drys(aer), Ns(aer), V, κs(aer), accom]
...
append!(state_atol, 1e-12*ones(length(aer.bins)))
state_rtol = 1e-7
